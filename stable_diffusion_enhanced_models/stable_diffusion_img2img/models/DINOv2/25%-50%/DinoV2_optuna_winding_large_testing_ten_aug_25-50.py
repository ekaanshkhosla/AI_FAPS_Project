...
image_dir_train = os.path.join(working_folder, "Augmented_data/25-50_SD_img2img")
image_dir = os.path.join(working_folder, "data_all")

train_df = pd.read_csv('data_labels/augmentation_img2img/25_50_augmented.csv')
...
for i in range(0, 10):
    ...
    model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=7.5e-07)
    loss_function = nn.BCEWithLogitsLoss()
    epochs_no_improve = 0
    num_epochs = 5
    best_val_f1 = 0
    best_model_path = "DinoV2_optuna_winding_large_testing_ten_aug_25-50_best_model.pth"
...
pd.DataFrame(all_scores).to_csv("DinoV2_optuna_winding_large_testing_ten_aug_25-50.csv")