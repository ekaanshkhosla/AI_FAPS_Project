...
train_df = pd.read_csv('data_labels/new_labels/splits/train_ten.csv')
...
def print_hyperparameters(image_size, batch_size, learning_rate, fc_units, fc_units_2, layer_freeze_upto):
    ...
    print(f"Freeze layer upto: {layer_freeze_upto}")
    
    
# Add classification head on DINOv2     
class CustomDINONormModel(nn.Module):
    def __init__(self, dino_model, fc_units, fc_units_2, layer_freeze_upto):
...
def define_model(fc_units, fc_units_2, layer_freeze_upto):
    ...
    model = CustomDINONormModel(dino_model, fc_units, fc_units_2, layer_freeze_upto)
    
    return model


# Use SQLite file for storing trails
storage_url = "sqlite:///DinoV2_optuna_winding_large_ten_freezing2.db"
study_name = "DinoV2_optuna_winding_large_ten_freezing2"
...
def objective(trial):
    ...
    fc_units_2 = trial.suggest_categorical('fc_units_2', [256, 512, 1024])
    layer_freeze_upto = trial.suggest_categorical('layer_freeze_upto', ['dino_model.blocks.0.ls2.gamma',
                                                                        ...
                                                                        'dino_model.blocks.7.ls2.gamma',
                                                                        'dino_model.blocks.8.ls2.gamma',
                                                                        'dino_model.blocks.9.ls2.gamma',
                                                                        'dino_model.blocks.10.ls2.gamma',
                                                                        'dino_model.blocks.11.ls2.gamma',])
    ...
    print_hyperparameters(image_size, batch_size, learning_rate, fc_units, fc_units_2, layer_freeze_upto)
    ...
    model = define_model(fc_units, fc_units_2, layer_freeze_upto)
    model.to(device)
